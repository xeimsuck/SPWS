...
spws::net::session::session(boost::asio::ip::tcp::socket &&socket) :
                    socket(std::move(socket)) {
}

void spws::net::session::run() {
    std::cout << "run\n";
    listen();
}

void spws::net::session::listen() {
...
void spws::net::session::request(const std::size_t size) {
    std::string str = "HTTP/1.1 200 OK\nContent-Type: text/html\r\nContent-Length: 52\r\n\r\n<html><head></head><body><h1>SPWS</h1></body></html>";
    boost::asio::write(socket, boost::asio::buffer(str));
    buffer.consume(buffer.size());
}

int spws::net::session::error_handler(boost::system::error_code error) {
    if(!error) return 0;
    switch (error.value()) {
